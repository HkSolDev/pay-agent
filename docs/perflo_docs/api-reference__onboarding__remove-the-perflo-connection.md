...
# Remove the Perflo connection

> Removes the customer's Perflo link and revokes the paired device on a best-effort basis. Admits a customer token with a fresh iat and answers 204 with no body. The removal erases the stored credential but keeps the account authority: mandates, agent pairings, beneficiaries, cards, quotes, and webhook subscriptions survive and re-enable when the same account connects again. Revoke a mandate before disconnecting if the customer wants it stopped, because revoking a beneficiary-payment mandate needs the credential this erases. It is refused while an operation still holds unsettled authority. After a lost or ambiguous response, the client must read GET /v1/onboarding before deciding whether to retry the delete or reconnect. An uncertain revocation retries automatically in the background rather than blocking on it.



## OpenAPI

````yaml /api-reference/openapi.json delete /v1/perflo-connections/current
openapi: 3.1.0
info:
  summary: >-
...
tags:
  ...
  - description: >-
    ...
    name: Webhooks
paths:
  /v1/perflo-connections/current:
    delete:
      tags:
        - Onboarding
      summary: Remove the Perflo connection
      description: >-
        Removes the customer's Perflo link and revokes the paired device on a
        best-effort basis. Admits a customer token with a fresh iat and answers
        204 with no body. The removal erases the stored credential but keeps the
        account authority: mandates, agent pairings, beneficiaries, cards,
        quotes, and webhook subscriptions survive and re-enable when the same
        account connects again. Revoke a mandate before disconnecting if the
        customer wants it stopped, because revoking a beneficiary-payment
        mandate needs the credential this erases. It is refused while an
        operation still holds unsettled authority. After a lost or ambiguous
        response, the client must read GET /v1/onboarding before deciding
        whether to retry the delete or reconnect. An uncertain revocation
        retries automatically in the background rather than blocking on it.
      operationId: disconnect_perflo_connection
      responses:
        '204':
