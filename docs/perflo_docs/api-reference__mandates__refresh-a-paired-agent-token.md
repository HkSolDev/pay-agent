...
# Refresh a paired agent token

> Re-stamps the pairing's token issue time and returns the same opaque pfa_ token. The token is never rotated, so a lost refresh response leaves the existing value usable.



## OpenAPI

````yaml /api-reference/openapi.json post /v1/agent-tokens/refresh
openapi: 3.1.0
info:
  summary: >-
...
paths:
  /v1/agent-tokens/refresh:
    post:
      tags:
        - Mandates
      summary: Refresh a paired agent token
      description: >-
        Re-stamps the pairing's token issue time and returns the same opaque
        pfa_ token. The token is never rotated, so a lost refresh response
        leaves the existing value usable.
      operationId: refresh_agent_token
      responses:
        '200':
          content:
            application/json:
              schema:
                $ref: '#/components/schemas/ConnectCodeRedemptionView'
...
components:
  schemas:
    ConnectCodeRedemptionView:
      additionalProperties: false
      description: The bearer credential created by presenting a valid connect code.
      properties:
        access_token:
          description: The opaque pairing bearer token. Returned only now.
          title: Access Token
          type: string
        expires_in:
          description: Seconds until refresh is required.
          exclusiveMinimum: 0
          title: Expires In
          type: integer
        mandate_id:
          description: The mandate the connect code named.
          title: Mandate Id
          type: string
        scopes:
          description: The authority derived from the mandate kind.
          items:
            type: string
          title: Scopes
          type: array
      required:
        ...
        - expires_in
        - scopes
      title: ConnectCodeRedemptionView
      type: object
    ProblemDetails:
      additionalProperties: false
