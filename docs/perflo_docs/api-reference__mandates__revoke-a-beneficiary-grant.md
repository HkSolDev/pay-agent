...
# Revoke a beneficiary grant

> Revokes one automatic-payment grant made directly on the customer's Perflo account and answers 202 with the operation that tracks it. The grant is named by the identifier GET /v1/mandates/beneficiary-grants publishes, and only a grant that route lists can be revoked here; a mandate's own grant is revoked with the mandate through POST /v1/mandates/{mandate_id}/revoke. Admits a customer token carrying a fresh iat, a Confirmation-Intent-ID naming the confirmed intent for this action, and an Idempotency-Key. Revocation is terminal, and no further payment is made under the grant. The Idempotency-Key header binds for at least the replay window published by GET /v1/identity. Inside that guaranteed window an equal replay returns the same operation with Idempotent-Replayed set to true. A client with an unresolved response must stop before the window ends and reconcile operation history rather than retry later. A transport-uncertain write leaves the operation indeterminate and is never repeated.



## OpenAPI

````yaml /api-reference/openapi.json post /v1/mandates/beneficiary-grants/{grant_id}/revoke
openapi: 3.1.0
info:
  summary: >-
...
tags:
  ...
  - description: >-
    ...
    name: Webhooks
paths:
  /v1/mandates/beneficiary-grants/{grant_id}/revoke:
    post:
      tags:
        - Mandates
      summary: Revoke a beneficiary grant
      description: >-
        Revokes one automatic-payment grant made directly on the customer's
        Perflo account and answers 202 with the operation that tracks it. The
        grant is named by the identifier GET /v1/mandates/beneficiary-grants
        publishes, and only a grant that route lists can be revoked here; a
        mandate's own grant is revoked with the mandate through POST
        /v1/mandates/{mandate_id}/revoke. Admits a customer token carrying a
        fresh iat, a Confirmation-Intent-ID naming the confirmed intent for this
        action, and an Idempotency-Key. Revocation is terminal, and no further
        payment is made under the grant. The Idempotency-Key header binds for at
      ...
      operationId: revoke_beneficiary_grant
      parameters:
        - in: path
          name: grant_id
          required: true
