...
# List card-account withdrawals

> Lists the customer's card-account withdrawals. Admits a customer token and answers synchronously. An optional card identifier scopes the result to that card.



## OpenAPI

````yaml /api-reference/openapi.json get /v1/card-account/withdrawals
openapi: 3.1.0
info:
  summary: >-
...
paths:
  /v1/card-account/withdrawals:
    get:
      tags:
        - Cards
      summary: List card-account withdrawals
      description: >-
        Lists the customer's card-account withdrawals. Admits a customer token
        and answers synchronously. An optional card identifier scopes the result
        to that card.
      operationId: card_withdrawals
      parameters:
        - in: query
          name: card_id
          required: false
        ...
        '200':
          content:
            application/json:
              schema:
                items:
                  $ref: '#/components/schemas/CardWithdrawalView'
                title: Response Card Withdrawals
                type: array
...
components:
  schemas:
    CardWithdrawalView:
      ...
      description: One withdrawal from the customer's card account.
      properties:
        amount:
          $ref: '#/components/schemas/NonNegativeMoney'
          description: The withdrawal's value in United States dollars.
        asset:
          description: The withdrawn asset, passed through unchanged.
          title: Asset
          type: string
        card_id:
          anyOf:
        ...
        completed_at:
          anyOf:
            - format: date-time
              type: string
            - type: 'null'
          description: When the withdrawal completed; null while completion is unstated.
          title: Completed At
        id:
          description: The identifier for the withdrawal.
          title: Id
          type: string
        requested_at:
        ...
        status:
          description: >-
            The withdrawal status, passed through unchanged. The set of values
            is open, so branch on the ones a client acts on and carry the rest
            through.
          title: Status
          type: string
        tx_hash:
          anyOf:
            - type: string
            - type: 'null'
          description: The transaction hash; null until one is stated.
          title: Tx Hash
      required:
        - id
        - card_id
