...
# List purchasable services

> Lists customer-safe service metadata. Agent calls require services:read and a mandate_id query parameter naming the active service-purchase mandate bound to that pairing and the current Perflo connection.



## OpenAPI

````yaml /api-reference/openapi.json get /v1/services
openapi: 3.1.0
info:
  summary: >-
...
paths:
  /v1/services:
    get:
      tags:
        - Services
      summary: List purchasable services
      description: >-
        Lists customer-safe service metadata. Agent calls require services:read
        and a mandate_id query parameter naming the active service-purchase
        mandate bound to that pairing and the current Perflo connection.
...
components:
  schemas:
    ServiceView:
      ...
      description: A customer-safe service catalogue entry.
      properties:
        capability:
          anyOf:
            - type: string
