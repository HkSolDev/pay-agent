...
# Read a purchasable service

> Returns one service and its input schema without exposing endpoint or settlement terms. Agent calls require a mandate_id query parameter naming current service-purchase authority.



## OpenAPI

````yaml /api-reference/openapi.json get /v1/services/{service_id}
openapi: 3.1.0
info:
  summary: >-
...
paths:
  /v1/services/{service_id}:
    get:
      tags:
        - Services
      summary: Read a purchasable service
      description: >-
        Returns one service and its input schema without exposing endpoint or
        settlement terms. Agent calls require a mandate_id query parameter
        naming current service-purchase authority.
      ...
      responses:
        '200':
          content:
            application/json:
              schema:
                $ref: '#/components/schemas/ServiceDetailView'
...
components:
  schemas:
    ServiceDetailView:
      ...
      description: A service catalogue entry with its declared input schema.
      properties:
        capability:
          anyOf:
            - type: string
            - type: 'null'
          description: The service capability; null when Perflo does not declare one.
          title: Capability
        description:
          anyOf:
            - type: string
            - type: 'null'
          description: The service description; null when none is declared.
          title: Description
        id:
          description: The service identifier accepted by a purchase target.
          title: Id
          type: string
        input_schema:
          anyOf:
            - additionalProperties: true
              type: object
            - type: 'null'
          description: The JSON input schema; null when the service declares none.
          title: Input Schema
        name:
        ...
        price:
          anyOf:
            ...
            - type: 'null'
          description: The observed USD price; null when a ranked result omits its chain.
        recommended:
          anyOf:
            - type: boolean
            - type: 'null'
          description: Whether Perflo ranked this service first; null for unranked entries.
          title: Recommended
        source:
          description: Whether this entry came from the catalogue or a ranked result.
          enum:
            - catalogue
            - ranked
          title: Source
        ...
        verified:
          anyOf:
            - type: boolean
            - type: 'null'
          description: Whether Perflo verified the service; null when it makes no claim.
          title: Verified
      required:
        - id
        - name
