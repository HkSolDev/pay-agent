...
# Read the display currency and rate

> Reads the customer's Perflo display currency and the conversion rate from the base currency. Admits a customer token and answers synchronously. A null body means amounts already read in the display currency and no conversion applies. The rate is an indicative display aid rather than an executable quote, and mandate caps and rolling limits stay United States dollars regardless.



## OpenAPI

````yaml /api-reference/openapi.json get /v1/display-currency
openapi: 3.1.0
info:
  summary: >-
...
paths:
  /v1/display-currency:
    get:
      tags:
        - Accounts
      summary: Read the display currency and rate
      description: >-
        Reads the customer's Perflo display currency and the conversion rate
        from the base currency. Admits a customer token and answers
        synchronously. A null body means amounts already read in the display
        currency and no conversion applies. The rate is an indicative display
        aid rather than an executable quote, and mandate caps and rolling limits
        stay United States dollars regardless.
        ...
        '200':
          content:
            application/json:
              schema:
                anyOf:
                  - $ref: '#/components/schemas/DisplayCurrencyView'
                  - type: 'null'
                title: Response Display Currency
...
components:
  schemas:
    DisplayCurrencyView:
      ...
      description: >-
        The currency the customer's Perflo profile displays amounts in, and the

        rate for converting the base currency into it.


        A null response body means no conversion applies. The rate is an
        indicative

        display aid: it never restates a balance as authority.
      properties:
        base_currency:
          description: >-
            The ISO 4217 alphabetic code the rate converts from, normalized to
            upper case.
          pattern: ^[A-Za-z]{3}$
          title: Base Currency
          type: string
        currency:
          description: >-
            The ISO 4217 alphabetic code amounts are displayed in, normalized to
            upper case.
          pattern: ^[A-Za-z]{3}$
          title: Currency
          type: string
        observed_at:
          description: When the rate was stated.
          format: date-time
          title: Observed At
          type: string
        units_per_base:
          description: >-
            How many display-currency units one base-currency unit buys, carried
            as a decimal string like every other money amount.
          pattern: ^\+?(?:\d*[1-9]\d*(?:\.\d*)?|\d*\.\d*[1-9]\d*)(?:[eE][+-]?\d+)?$
          title: Units Per Base
          type: string
      required:
        - currency
        - base_currency
