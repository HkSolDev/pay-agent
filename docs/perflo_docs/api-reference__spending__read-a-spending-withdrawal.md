...
# Read a spending withdrawal

> Returns the signed-in customer's durable withdrawal status and the settled amount once Perflo reports it.



## OpenAPI

````yaml /api-reference/openapi.json get /v1/spending-withdrawals/{withdrawal_id}
openapi: 3.1.0
info:
  summary: >-
...
tags:
  ...
  - description: >-
    ...
    name: Webhooks
paths:
  /v1/spending-withdrawals/{withdrawal_id}:
    get:
      tags:
        - Spending
      summary: Read a spending withdrawal
      description: >-
        Returns the signed-in customer's durable withdrawal status and the
        settled amount once Perflo reports it.
      operationId: get_spending_withdrawal
      parameters:
        - in: path
          name: withdrawal_id
          required: true
      ...
      responses:
        '200':
          content:
            application/json:
              schema:
                $ref: '#/components/schemas/SpendingWithdrawalView'
...
components:
  schemas:
    SpendingWithdrawalView:
      additionalProperties: false
      description: The durable status of one held-funds withdrawal.
      properties:
        completed_at:
          anyOf:
            - format: date-time
              type: string
            - type: 'null'
          description: When the withdrawal reached a terminal state; null while open.
          title: Completed At
        created_at:
          description: When the withdrawal was accepted.
          format: date-time
          title: Created At
          type: string
        id:
        ...
        requested:
          anyOf:
            ...
            - type: 'null'
          description: The requested amount; null means every withdrawable held fund.
        settled:
          anyOf:
            - $ref: '#/components/schemas/NonNegativeMoney'
            - type: 'null'
          description: >-
            The exact settled amount when known; zero only when every submitted
            leg and bridge phase definitively failed or was skipped; null when
            Perflo does not prove an amount.
        status:
          description: The withdrawal lifecycle status.
          enum:
            - queued
            - pending
            - bridging
