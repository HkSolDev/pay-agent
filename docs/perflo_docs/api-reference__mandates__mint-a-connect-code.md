...
> Mints a short-lived, single-use connect code for an active service-purchase or beneficiary-payment mandate. Presenting the code is the authorization to create one pairing, so transfer it out of band and treat it as a credential. It expires in ten minutes, and beneficiary authority must already exist.



## OpenAPI

````yaml /api-reference/openapi.json post /v1/mandates/{mandate_id}/connect-codes
openapi: 3.1.0
info:
  summary: >-
...
tags:
  ...
  - description: >-
    ...
    name: Webhooks
paths:
  /v1/mandates/{mandate_id}/connect-codes:
    post:
      tags:
        - Mandates
      summary: Mint a connect code
      description: >-
        Mints a short-lived, single-use connect code for an active
        service-purchase or beneficiary-payment mandate. Presenting the code is
        the authorization to create one pairing, so transfer it out of band and
        treat it as a credential. It expires in ten minutes, and beneficiary
        authority must already exist.
      operationId: create_connect_code
      parameters:
        - in: path
          name: mandate_id
          required: true
      ...
      responses:
        '201':
          content:
            application/json:
              schema:
                $ref: '#/components/schemas/ConnectCodeView'
...
components:
  schemas:
    ConnectCodeView:
      ...
      description: |-
        A short-lived, single-use selector an agent redeems to learn its
        mandate_id, so the customer never has to hand it a raw UUID.
      properties:
        code:
          description: The connect code to give the agent out of band.
          title: Code
          type: string
        expires_at:
          description: When this code stops being redeemable.
          format: date-time
          title: Expires At
          type: string
        mandate_id:
          description: The mandate this code resolves to.
          title: Mandate Id
          type: string
      required:
        - code
