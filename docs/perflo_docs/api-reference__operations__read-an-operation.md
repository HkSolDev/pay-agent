...
> Returns one operation's current state. Two caller classes are admitted: the customer who owns the operation, or an agent token that carries the mandates:read, purchases:read, or purchases:execute scope and whose subject and client identifier both match the actor recorded on the operation. Any other caller is refused. Answers synchronously from local state. A customer read carries the hosted approval hand-off while one is outstanding; an agent read never does.



## OpenAPI

````yaml /api-reference/openapi.json get /v1/operations/{operation_id}
openapi: 3.1.0
info:
  summary: >-
...
paths:
  /v1/operations/{operation_id}:
    get:
      ...
      description: >-
        Returns one operation's current state. Two caller classes are admitted:
        the customer who owns the operation, or an agent token that carries the
        mandates:read, purchases:read, or purchases:execute scope and whose
        subject and client identifier both match the actor recorded on the
        operation. Any other caller is refused. Answers synchronously from local
        state. A customer read carries the hosted approval hand-off while one is
        outstanding; an agent read never does.
