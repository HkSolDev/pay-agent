...
> Records a mandate, the bounded payment authority this customer delegates through an agent pairing or rule, and answers 202 with the operation that tracks it. Admits a customer token carrying a fresh iat, a Confirmation-Intent-ID naming the confirmed intent for this action, and an Idempotency-Key. Every cap is a Perflo cash United States dollar amount. The beneficiary-payment kind starts pending_approval and carries a hosted approval hand-off; its remaining expiry is floored to whole days, so the authority behind it expires at or before the mandate does. The service-purchase kind becomes active synchronously after confirmation and is bounded by its pairing, allowlists, amount, count and rolling limits. The Idempotency-Key header binds for at least the replay window published by GET /v1/identity. Inside that guaranteed window an equal replay returns the same operation with Idempotent-Replayed set to true. A client with an unresolved response must stop before the window ends and reconcile operation history rather than retry later. A transport-uncertain write leaves the operation indeterminate and is never repeated.



## OpenAPI

````yaml /api-reference/openapi.json post /v1/mandates
openapi: 3.1.0
info:
  summary: >-
...
paths:
  /v1/mandates:
    post:
      ...
      description: >-
        Records a mandate, the bounded payment authority this customer delegates
        through an agent pairing or rule, and answers 202 with the operation
        that tracks it. Admits a customer token carrying a fresh iat, a
        Confirmation-Intent-ID naming the confirmed intent for this action, and
        an Idempotency-Key. Every cap is a Perflo cash United States dollar
        amount. The beneficiary-payment kind starts pending_approval and carries
        a hosted approval hand-off; its remaining expiry is floored to whole
        days, so the authority behind it expires at or before the mandate does.
        The service-purchase kind becomes active synchronously after
        confirmation and is bounded by its pairing, allowlists, amount, count
        and rolling limits. The Idempotency-Key header binds for at least the
        replay window published by GET /v1/identity. Inside that guaranteed
        window an equal replay returns the same operation with
          ...
          application/json:
            schema:
              discriminator:
                mapping:
                  beneficiary_payment:
                    $ref: '#/components/schemas/BeneficiaryMandateCreate'
                  service_purchase:
                    $ref: '#/components/schemas/ServiceMandateCreate'
                propertyName: kind
              oneOf:
                - $ref: '#/components/schemas/BeneficiaryMandateCreate'
                - $ref: '#/components/schemas/ServiceMandateCreate'
...
components:
  schemas:
    BeneficiaryMandateCreate:
      additionalProperties: false
      description: Bounded authority to pay one existing beneficiary.
      properties:
        authorized_rules:
          ...
          type: array
        beneficiary_id:
          description: The single beneficiary every execution under this mandate pays.
        ...
        daily_max:
          ...
          type: string
        expires_at:
          description: When the authority ends. The value must carry a UTC offset.
          format: date-time
          title: Expires At
          type: string
        kind:
          const: beneficiary_payment
          description: Selects beneficiary-payment authority.
          title: Kind
          type: string
        monthly_max:
          description: >-
      ...
      required:
        ...
        - beneficiary_id
        - authorized_rules
      title: BeneficiaryMandateCreate
      type: object
    ServiceMandateCreate:
      additionalProperties: false
      description: Bounded authority to purchase live services.
      properties:
        allowed_capabilities:
          anyOf:
            ...
            - type: 'null'
          description: >-
            The capabilities this authority may buy; null leaves capabilities
            unrestricted.
        ...
        allowed_services:
          anyOf:
            ...
            - type: 'null'
          description: >-
            The service identifiers this authority may buy; null leaves services
            unrestricted.
        ...
        expires_at:
          ...
          type: string
        kind:
          const: service_purchase
          description: Selects service-purchase authority.
          title: Kind
          type: string
        monthly_max:
          description: >-
