...
# Start a Perflo connection

> Starts the device link between this customer and their Perflo account. Admits a customer token with a fresh iat. Answers synchronously with a hosted connect action on the trusted Perflo application origin for the browser to open, and records no operation. Advance the link with POST /v1/perflo-connections/current/poll.



## OpenAPI

````yaml /api-reference/openapi.json post /v1/perflo-connections
openapi: 3.1.0
info:
  summary: >-
...
paths:
  /v1/perflo-connections:
    post:
      tags:
        - Onboarding
      summary: Start a Perflo connection
      description: >-
        Starts the device link between this customer and their Perflo account.
        Admits a customer token with a fresh iat. Answers synchronously with a
        hosted connect action on the trusted Perflo application origin for the
        browser to open, and records no operation. Advance the link with POST
        /v1/perflo-connections/current/poll.
      operationId: start_perflo_connection
      responses:
        '201':
          content:
            application/json:
