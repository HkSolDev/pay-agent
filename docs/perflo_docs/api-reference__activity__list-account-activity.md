...
> Lists a customer-wide activity projection merged from two sources of movement, deduplicated, ordered newest first and truncated to limit. Admits a customer token and answers synchronously. The projection is not a per-account ledger: an entry is not attributable to one account.



## OpenAPI

````yaml /api-reference/openapi.json get /v1/activity
openapi: 3.1.0
info:
  summary: >-
...
paths:
  /v1/activity:
    get:
      ...
      description: >-
        Lists a customer-wide activity projection merged from two sources of
        movement, deduplicated, ordered newest first and truncated to limit.
        Admits a customer token and answers synchronously. The projection is not
        a per-account ledger: an entry is not attributable to one account.
...
components:
  schemas:
    ActivityPage:
      ...
      description: >-
        The customer-wide activity projection, ordered newest first.


        The projection merges two sources of movement and deduplicates them, so
        it is not

        a per-account ledger and an entry is not attributable to one account.
      properties:
        items:
          description: The entries, ordered newest first.
          items:
    ...
    ActivityView:
      additionalProperties: false
      description: >-
        One entry of the customer-wide activity projection.
      ...
      properties:
        amount:
          ...
          description: The entry's amount, always non-negative; kind carries the direction.
        description:
          description: Display text for the entry, as supplied.
          title: Description
          type: string
        id:
          description: The identifier for this entry within the projection.
          title: Id
          type: string
        kind:
          description: >-
            What the entry is. Direction is carried by kind, not by a sign, so
            amount stays non-negative.
          enum:
            - deposit
            - withdrawal
            - payment
        ...
        status:
          description: >-
            The entry's own status, passed through unchanged, so the vocabulary
            is open.
          title: Status
          type: string
      required:
