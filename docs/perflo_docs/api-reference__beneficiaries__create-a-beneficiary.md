...
> Records a beneficiary creation and answers 202 with the operation that tracks it. Admits a customer token without requiring a fresh iat: this is the one financial mutation where that is a deliberate choice rather than an omission. The Idempotency-Key header binds for at least the replay window published by GET /v1/identity. Inside that guaranteed window an equal replay returns the same operation with Idempotent-Replayed set to true. A client with an unresolved response must stop before the window ends and reconcile operation history rather than retry later. A transport-uncertain write leaves the operation indeterminate and is never repeated. The beneficiary becomes readable through GET /v1/beneficiaries when the operation succeeds.



## OpenAPI

````yaml /api-reference/openapi.json post /v1/beneficiaries
openapi: 3.1.0
info:
  summary: >-
...
paths:
  /v1/beneficiaries:
    post:
      ...
      description: >-
        Records a beneficiary creation and answers 202 with the operation that
        tracks it. Admits a customer token without requiring a fresh iat: this
        is the one financial mutation where that is a deliberate choice rather
        than an omission. The Idempotency-Key header binds for at least the
        ...
        indeterminate and is never repeated. The beneficiary becomes readable
        through GET /v1/beneficiaries when the operation succeeds.
...
components:
  schemas:
    BeneficiaryCreate:
      ...
      description: The request that records a payee the customer can send to.
      properties:
        country:
          description: >-
            The ISO 3166-1 alpha-2 payout country, which selects the rail
            together with currency and payout_schema_id.
          maxLength: 2
          minLength: 2
          pattern: ^[A-Za-z]{2}$
          title: Country
          type: string
        currency:
          description: The ISO 4217 alphabetic payout currency.
          maxLength: 3
          minLength: 3
          pattern: ^[A-Za-z]{3}$
          title: Currency
        ...
        details:
          ...
          description: >-
            The answers to the payout schema's fields, keyed by each field's
            key.
          title: Details
          type: object
        is_external:
          ...
          type: boolean
        name:
          description: The payee's own name, exactly as the payout form requires it.
          maxLength: 200
          minLength: 1
          title: Name
          type: string
        nickname:
          anyOf:
            ...
            - type: 'null'
          description: >-
            The customer's private label for the beneficiary, trimmed and at
            most 80 characters; a blank value is null.
          title: Nickname
        payout_schema_id:
          description: The payout schema whose fields details answers.
          maxLength: 100
          minLength: 1
        ...
        purpose_code:
          anyOf:
            ...
            - type: 'null'
          description: >-
            The purpose code, when the payout form requires one; null when none
            is required.
          title: Purpose Code
      required:
        - name
