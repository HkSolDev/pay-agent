...
> Revokes one of the customer's mandates and answers 202 with the operation that tracks it. Admits a customer token carrying a fresh iat, a Confirmation-Intent-ID naming the confirmed intent for this action, and an Idempotency-Key. Revocation is a transition rather than a deletion: the mandate record survives with its audit trail. A beneficiary-payment mandate moves to revocation_pending while the exact grant is revoked. A service-purchase mandate becomes revoked locally in the same transaction. Both paths stop new executions and cancel unsent reservations; neither has a resume path. The Idempotency-Key header binds for at least the replay window published by GET /v1/identity. Inside that guaranteed window an equal replay returns the same operation with Idempotent-Replayed set to true. A client with an unresolved response must stop before the window ends and reconcile operation history rather than retry later. A transport-uncertain write leaves the operation indeterminate and is never repeated.



## OpenAPI

````yaml /api-reference/openapi.json post /v1/mandates/{mandate_id}/revoke
openapi: 3.1.0
info:
  summary: >-
...
paths:
  /v1/mandates/{mandate_id}/revoke:
    post:
      ...
      description: >-
        Revokes one of the customer's mandates and answers 202 with the
        ...
        and an Idempotency-Key. Revocation is a transition rather than a
        deletion: the mandate record survives with its audit trail. A
        beneficiary-payment mandate moves to revocation_pending while the exact
        grant is revoked. A service-purchase mandate becomes revoked locally in
        the same transaction. Both paths stop new executions and cancel unsent
        reservations; neither has a resume path. The Idempotency-Key header
        binds for at least the replay window published by GET /v1/identity.
        Inside that guaranteed window an equal replay returns the same operation
        with Idempotent-Replayed set to true. A client with an unresolved
        response must stop before the window ends and reconcile operation
        history rather than retry later. A transport-uncertain write leaves the
        operation indeterminate and is never repeated.
