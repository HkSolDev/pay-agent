...
> Lists the beneficiaries this customer can send to. Admits a customer token and answers synchronously. Every field is emitted on every row, so a null is the beneficiary's own absence of a value rather than an omission by this read.



## OpenAPI

````yaml /api-reference/openapi.json get /v1/beneficiaries
openapi: 3.1.0
info:
  summary: >-
...
paths:
  /v1/beneficiaries:
    get:
      ...
      description: >-
        Lists the beneficiaries this customer can send to. Admits a customer
        token and answers synchronously. Every field is emitted on every row, so
        a null is the beneficiary's own absence of a value rather than an
        omission by this read.
