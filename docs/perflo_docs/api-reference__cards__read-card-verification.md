...
# Read card verification

> Reads the customer's normalized card-verification status, its stated value and whether a card can be issued. Admits a customer token and answers synchronously.



## OpenAPI

````yaml /api-reference/openapi.json get /v1/card-account/kyc
openapi: 3.1.0
info:
  summary: >-
...
paths:
  /v1/card-account/kyc:
    get:
      tags:
        - Cards
      summary: Read card verification
      description: >-
        Reads the customer's normalized card-verification status, its stated
        value and whether a card can be issued. Admits a customer token and
        answers synchronously.
      operationId: card_kyc_status
      responses:
        '200':
          content:
            application/json:
              schema:
                $ref: '#/components/schemas/CardKycView'
...
components:
  schemas:
    CardKycView:
      ...
      description: The customer's card-verification status and issuance eligibility.
      properties:
        can_issue_card:
          description: >-
            Whether the customer can issue a card in the current verification
            state.
          title: Can Issue Card
          type: boolean
        raw_status:
          anyOf:
            - type: string
