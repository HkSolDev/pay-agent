...
> Reads one of the customer's cards by its card identifier. Admits a customer token and answers synchronously. A card with a write in flight answers from its stored state so a read cannot overwrite that state.



## OpenAPI

````yaml /api-reference/openapi.json get /v1/cards/{card_id}
openapi: 3.1.0
info:
  summary: >-
...
paths:
  /v1/cards/{card_id}:
    get:
      ...
      summary: Read a card
      description: >-
        Reads one of the customer's cards by its card identifier. Admits a
        customer token and answers synchronously. A card with a write in flight
        answers from its stored state so a read cannot overwrite that state.
      operationId: card
      parameters:
        - in: path
          name: card_id
