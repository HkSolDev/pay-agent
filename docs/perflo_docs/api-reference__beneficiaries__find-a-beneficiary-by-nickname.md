...
# Find a beneficiary by nickname

> Returns the customer's beneficiary carrying the exact private label. Admits a customer token, answers synchronously, and answers 404 when no current beneficiary can be joined to that label.



## OpenAPI

````yaml /api-reference/openapi.json get /v1/beneficiaries/by-nickname/{nickname}
openapi: 3.1.0
info:
  summary: >-
...
tags:
  ...
  - description: >-
    ...
    name: Webhooks
paths:
  /v1/beneficiaries/by-nickname/{nickname}:
    get:
      tags:
        - Beneficiaries
      summary: Find a beneficiary by nickname
      description: >-
        Returns the customer's beneficiary carrying the exact private label.
        Admits a customer token, answers synchronously, and answers 404 when no
        current beneficiary can be joined to that label.
      operationId: beneficiary_by_nickname
      parameters:
        - in: path
          name: nickname
          required: true
