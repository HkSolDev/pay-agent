...
# Poll an operation approval

> Reconciles a browser approval for one operation into local state and returns the operation. Admits a customer token and answers synchronously. The route is safe to repeat by design and therefore takes no idempotency key.



## OpenAPI

````yaml /api-reference/openapi.json post /v1/operations/{operation_id}/approval/poll
openapi: 3.1.0
info:
  summary: >-
...
tags:
  ...
  - description: >-
    ...
    name: Webhooks
paths:
  /v1/operations/{operation_id}/approval/poll:
    post:
      tags:
        - Operations
      summary: Poll an operation approval
      description: >-
        Reconciles a browser approval for one operation into local state and
        returns the operation. Admits a customer token and answers
        synchronously. The route is safe to repeat by design and therefore takes
        no idempotency key.
      operationId: poll_operation_approval
      parameters:
        - in: path
          name: operation_id
          required: true
