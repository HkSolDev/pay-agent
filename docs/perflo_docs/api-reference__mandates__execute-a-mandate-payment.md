...
> Makes one payment under an existing mandate and answers 202 with the operation that tracks it. Two caller classes are admitted: the customer who owns the mandate, carrying a fresh iat and a one-use confirmation naming this mandate and amount, or an agent token carrying the mandates:execute scope whose client identifier the mandate's authorized clients name, or whose client identifier and submitted rule_id exactly match an authorized rule; no other caller can spend a mandate. A customer payment carries no rule_id and needs no browser approval, because the mandate's Perflo grant already exists. The payment count, total, daily, weekly and monthly capacity is reserved atomically before the payment is submitted. Immediately before paying, the exact beneficiary grant backing the mandate is rechecked, and the mandate is disabled instead when the grant no longer matches. The Idempotency-Key header binds for at least the replay window published by GET /v1/identity. Inside that guaranteed window an equal replay returns the same operation with Idempotent-Replayed set to true. A client with an unresolved response must stop before the window ends and reconcile operation history rather than retry later. A transport-uncertain write leaves the operation indeterminate and is never repeated.



## OpenAPI

````yaml /api-reference/openapi.json post /v1/mandates/{mandate_id}/executions
openapi: 3.1.0
info:
  summary: >-
...
tags:
  ...
  - description: >-
    ...
    name: Webhooks
paths:
  /v1/mandates/{mandate_id}/executions:
    post:
      tags:
        - Mandates
      summary: Execute a mandate payment
      description: >-
        Makes one payment under an existing mandate and answers 202 with the
        operation that tracks it. Two caller classes are admitted: the customer
        who owns the mandate, carrying a fresh iat and a one-use confirmation
        naming this mandate and amount, or an agent token carrying the
        mandates:execute scope whose client identifier the mandate's authorized
        clients name, or whose client identifier and submitted rule_id exactly
        match an authorized rule; no other caller can spend a mandate. A
        customer payment carries no rule_id and needs no browser approval,
        because the mandate's Perflo grant already exists. The payment count,
        total, daily, weekly and monthly capacity is reserved atomically before
        the payment is submitted. Immediately before paying, the exact
        beneficiary grant backing the mandate is rechecked, and the mandate is
        disabled instead when the grant no longer matches. The Idempotency-Key
        header binds for at least the replay window published by GET
        /v1/identity. Inside that guaranteed window an equal replay returns the
        same operation with Idempotent-Replayed set to true. A client with an
        unresolved response must stop before the window ends and reconcile
      ...
      requestBody:
        content:
          application/json:
            schema:
              $ref: '#/components/schemas/MandateExecutionCreate'
        required: true
      responses:
        '202':
          content:
...
components:
  schemas:
    MandateExecutionCreate:
      additionalProperties: false
      description: The request that makes one payment under an existing mandate.
      properties:
        amount:
        ...
        rule_id:
          anyOf:
            ...
            - type: 'null'
          description: >-
            The optional agent rule making the payment; authority requires an
            exact bare rule-id mandate binding.
          title: Rule Id
      required:
        - amount
      title: MandateExecutionCreate
      type: object
    OperationView:
      additionalProperties: false
      description: >-
