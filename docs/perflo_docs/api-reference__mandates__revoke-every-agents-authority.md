...
# Revoke every agent's authority

> Stops every agent's authority on the account in one call. Every active pairing is revoked immediately, and one mandate revocation opens for each mandate that still holds authority: a service-purchase mandate becomes revoked in the same transaction, and a beneficiary-payment mandate moves to revocation_pending while its exact grant is revoked. Admits a customer token carrying a fresh iat, a Confirmation-Intent-ID naming the confirmed intent for this action, and an Idempotency-Key. Answers 202 with the batch, the operation opened for each revocation, and the revoked pairing identifiers; the batch records which revocations were opened, and each operation carries its own outcome. A mandate already revoked, awaiting approval, refused at approval, awaiting revocation, or holding no exact authority opens no revocation, and neither does a service-purchase mandate past its expiry, whose expiry already ended its authority; any pairing it has is cut all the same. Revocation is terminal: no path re-grants the authority. An equal replay under the same Idempotency-Key returns the same batch. The Idempotency-Key header binds for at least the replay window published by GET /v1/identity. Inside that guaranteed window an equal replay returns the same operation with Idempotent-Replayed set to true. A client with an unresolved response must stop before the window ends and reconcile operation history rather than retry later. A transport-uncertain write leaves the operation indeterminate and is never repeated.



## OpenAPI

````yaml /api-reference/openapi.json post /v1/mandates/revoke-all
openapi: 3.1.0
info:
  summary: >-
...
tags:
  ...
  - description: >-
    ...
    name: Webhooks
paths:
  /v1/mandates/revoke-all:
    post:
      tags:
        - Mandates
      summary: Revoke every agent's authority
      description: >-
        Stops every agent's authority on the account in one call. Every active
        pairing is revoked immediately, and one mandate revocation opens for
        each mandate that still holds authority: a service-purchase mandate
        becomes revoked in the same transaction, and a beneficiary-payment
        mandate moves to revocation_pending while its exact grant is revoked.
        Admits a customer token carrying a fresh iat, a Confirmation-Intent-ID
        naming the confirmed intent for this action, and an Idempotency-Key.
        Answers 202 with the batch, the operation opened for each revocation,
        and the revoked pairing identifiers; the batch records which revocations
        were opened, and each operation carries its own outcome. A mandate
        already revoked, awaiting approval, refused at approval, awaiting
        revocation, or holding no exact authority opens no revocation, and
        neither does a service-purchase mandate past its expiry, whose expiry
        already ended its authority; any pairing it has is cut all the same.
        Revocation is terminal: no path re-grants the authority. An equal replay
        under the same Idempotency-Key returns the same batch. The
        Idempotency-Key header binds for at least the replay window published by
        GET /v1/identity. Inside that guaranteed window an equal replay returns
        the same operation with Idempotent-Replayed set to true. A client with
        an unresolved response must stop before the window ends and reconcile
        operation history rather than retry later. A transport-uncertain write
        leaves the operation indeterminate and is never repeated.
      operationId: revoke_all_mandates
      parameters:
      ...
      responses:
        '202':
          content:
            application/json:
              schema:
                $ref: '#/components/schemas/MandateRevocationBatchView'
...
components:
  schemas:
    MandateRevocationBatchView:
      additionalProperties: false
      description: >-
        The record of one account-wide revocation of every agent's authority.


        The batch is settled the moment it is answered, and it names the
        revocations it

        opened rather than their outcome: every pairing it lists is already
        revoked, while

        each mandate revocation carries its own state and a beneficiary-payment
        revocation

        settles only once its exact grant is revoked.
      properties:
        mandate_operations:
          description: >-
            One operation for each mandate revocation the batch opened, in the
            order the batch recorded them; empty when no mandate still held
            authority.
          items:
        ...
        operation:
          $ref: '#/components/schemas/OperationView'
          description: The operation recording the batch itself, already settled.
        revoked_pairing_ids:
          description: >-
            The pairings the batch revoked, named by the identifier GET
            /v1/mandates publishes in authorized_clients; empty when none was
            active.
          items:
            type: string
          title: Revoked Pairing Ids
          type: array
      required:
        - operation
        - mandate_operations
        - revoked_pairing_ids
      title: MandateRevocationBatchView
      type: object
    ProblemDetails:
      additionalProperties: false
