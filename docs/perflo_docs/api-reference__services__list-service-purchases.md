...
# List service purchases

> Lists the customer's service purchases, newest first. Admits a customer token and answers synchronously from local state. Filter to one mandate's purchases with mandate_id.



## OpenAPI

````yaml /api-reference/openapi.json get /v1/purchases
openapi: 3.1.0
info:
  summary: >-
...
paths:
  /v1/purchases:
    get:
      tags:
        - Services
      summary: List service purchases
      description: >-
        Lists the customer's service purchases, newest first. Admits a customer
        token and answers synchronously from local state. Filter to one
        mandate's purchases with mandate_id.
