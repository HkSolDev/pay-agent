...
> Marks a customer-owned webhook subscription revoked. A delivery checked after revocation is dead-lettered without an outbound request, but a worker that already passed the revocation check can still send one request after the 204 response.



## OpenAPI

````yaml /api-reference/openapi.json delete /v1/webhook-subscriptions/{subscription_id}
openapi: 3.1.0
info:
  summary: >-
...
paths:
  /v1/webhook-subscriptions/{subscription_id}:
    delete:
      ...
      description: >-
        Marks a customer-owned webhook subscription revoked. A delivery checked
        after revocation is dead-lettered without an outbound request, but a
        worker that already passed the revocation check can still send one
        request after the 204 response.
