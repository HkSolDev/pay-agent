...
paths:
  /v1/cards/{card_id}/unfreeze:
    post:
      ...
      description: >-
        Records an unfreeze on one of the customer's cards and answers 202 with
        the operation that tracks it. Admits a customer token carrying a fresh
        iat, a Confirmation-Intent-ID naming the confirmed intent for this
        action, and an Idempotency-Key. The card takes unfreeze_pending while
        the write is submitted; a confirmed failure restores the previous
        status, and a transport-uncertain write leaves the card and its
        operation indeterminate and is never repeated. The Idempotency-Key
