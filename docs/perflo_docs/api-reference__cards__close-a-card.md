...
> Records a close on one of the customer's cards and answers 202 with the operation that tracks it. Closing a card is irreversible. Admits a customer token carrying a fresh iat, a Confirmation-Intent-ID naming the confirmed intent for this action, and an Idempotency-Key. The card takes close_pending while the write is submitted; a confirmed failure restores the previous status, and a transport-uncertain write leaves the card and its operation indeterminate and is never repeated. The Idempotency-Key header binds for at least the replay window published by GET /v1/identity. Inside that guaranteed window an equal replay returns the same operation with Idempotent-Replayed set to true. A client with an unresolved response must stop before the window ends and reconcile operation history rather than retry later.



## OpenAPI

````yaml /api-reference/openapi.json post /v1/cards/{card_id}/close
openapi: 3.1.0
info:
  summary: >-
...
paths:
  /v1/cards/{card_id}/close:
    post:
      ...
      description: >-
        Records a close on one of the customer's cards and answers 202 with the
        operation that tracks it. Closing a card is irreversible. Admits a
        customer token carrying a fresh iat, a Confirmation-Intent-ID naming the
        confirmed intent for this action, and an Idempotency-Key. The card takes
        close_pending while the write is submitted; a confirmed failure restores
        the previous status, and a transport-uncertain write leaves the card and
        its operation indeterminate and is never repeated. The Idempotency-Key
        ...
        operation history rather than retry later.
      operationId: close_card
      parameters:
        - in: path
          name: card_id
          required: true
