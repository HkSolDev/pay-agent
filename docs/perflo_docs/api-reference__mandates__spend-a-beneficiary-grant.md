...
# Spend a beneficiary grant

> Makes one payment against automatic-payment authority that already exists on the customer's Perflo account rather than as a mandate, and answers 202 with the operation that tracks it. The grant is named by the identifier GET /v1/mandates/beneficiary-grants publishes. Admits a customer token carrying a fresh iat, a Confirmation-Intent-ID naming the confirmed intent for this action, and an Idempotency-Key. The request names the saved beneficiary the payment must reach, and the payment is submitted only while this grant is still active, unexpired, has a payment left, allows the amount, and pays exactly that beneficiary's destination -- checked once when the operation opens and again immediately before submission. This grant can be revoked with POST /v1/mandates/beneficiary-grants/{grant_id}/revoke, or in the Perflo app. No reservation ledger is kept against it and no mandate records it, so concurrent spending by another client sharing the account is bounded only by the grant's own caps. The Idempotency-Key header binds for at least the replay window published by GET /v1/identity. Inside that guaranteed window an equal replay returns the same operation with Idempotent-Replayed set to true. A client with an unresolved response must stop before the window ends and reconcile operation history rather than retry later. A transport-uncertain write leaves the operation indeterminate and is never repeated.



## OpenAPI

````yaml /api-reference/openapi.json post /v1/mandates/beneficiary-grants/{grant_id}/payments
openapi: 3.1.0
info:
  summary: >-
...
tags:
  ...
  - description: >-
    ...
    name: Webhooks
paths:
  /v1/mandates/beneficiary-grants/{grant_id}/payments:
    post:
      tags:
        - Mandates
      summary: Spend a beneficiary grant
      description: >-
        Makes one payment against automatic-payment authority that already
        exists on the customer's Perflo account rather than as a mandate, and
        answers 202 with the operation that tracks it. The grant is named by the
        identifier GET /v1/mandates/beneficiary-grants publishes. Admits a
        customer token carrying a fresh iat, a Confirmation-Intent-ID naming the
        confirmed intent for this action, and an Idempotency-Key. The request
        names the saved beneficiary the payment must reach, and the payment is
        submitted only while this grant is still active, unexpired, has a
        payment left, allows the amount, and pays exactly that beneficiary's
        destination -- checked once when the operation opens and again
        immediately before submission. This grant can be revoked with POST
        /v1/mandates/beneficiary-grants/{grant_id}/revoke, or in the Perflo app.
        No reservation ledger is kept against it and no mandate records it, so
        concurrent spending by another client sharing the account is bounded
        only by the grant's own caps. The Idempotency-Key header binds for at
        least the replay window published by GET /v1/identity. Inside that
        guaranteed window an equal replay returns the same operation with
      ...
      operationId: spend_beneficiary_grant
      parameters:
        - in: path
          name: grant_id
          required: true
      ...
      requestBody:
        content:
          application/json:
            schema:
              $ref: '#/components/schemas/BeneficiaryGrantPaymentCreate'
        required: true
      responses:
        '202':
          content:
...
components:
  schemas:
    BeneficiaryGrantPaymentCreate:
      additionalProperties: false
      description: >-
        The request that spends one payment of a beneficiary grant.


        The beneficiary names the destination the customer means to pay. The
        grant is

        checked immediately before submitting to confirm it still pays exactly
        that

        destination, and the payment is refused on any drift.
      properties:
        amount:
          description: The exact United States dollar debit for this payment.
          pattern: ^\+?(?:\d*[1-9]\d*(?:\.\d*)?|\d*\.\d*[1-9]\d*)(?:[eE][+-]?\d+)?$
          title: Amount
          type: string
        beneficiary_id:
          description: >-
            The saved beneficiary this payment must reach. The grant is spent
            only while it still pays this beneficiary's payout destination.
          minLength: 1
      ...
      required:
        - beneficiary_id
        - amount
      title: BeneficiaryGrantPaymentCreate
      type: object
    OperationView:
      additionalProperties: false
      description: >-
