...
> Admits a customer token and answers synchronously with the hosted identity-verification hand-off: the verification page Perflo states for this customer or, when none is stated, the verification page on the configured Perflo application origin, recording no operation. Answers 503 when the linked Perflo connection does not carry the verification capability.



## OpenAPI

````yaml /api-reference/openapi.json post /v1/kyc/sessions
openapi: 3.1.0
info:
  summary: >-
...
paths:
  /v1/kyc/sessions:
    post:
      ...
      description: >-
        Admits a customer token and answers synchronously with the hosted
        identity-verification hand-off: the verification page Perflo states for
        this customer or, when none is stated, the verification page on the
        configured Perflo application origin, recording no operation. Answers
        503 when the linked Perflo connection does not carry the verification
        capability.
      operationId: create_kyc_session
      responses:
        '201':
          content:
            application/json:
