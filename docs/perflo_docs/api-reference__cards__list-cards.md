...
> Lists the customer's cards. Admits a customer token and answers synchronously. A card with a local write in flight keeps its in-flight status rather than being overwritten by a stale read.



## OpenAPI

````yaml /api-reference/openapi.json get /v1/cards
openapi: 3.1.0
info:
  summary: >-
...
paths:
  /v1/cards:
    get:
      ...
      summary: List cards
      description: >-
        Lists the customer's cards. Admits a customer token and answers
        synchronously. A card with a local write in flight keeps its in-flight
        status rather than being overwritten by a stale read.
      operationId: cards
      responses:
        '200':
          content:
