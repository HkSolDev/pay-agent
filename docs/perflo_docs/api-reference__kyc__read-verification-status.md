...
> Returns the normalized identity-verification status, which this route is the single source of. Admits a customer token and answers synchronously. An unrecognized or absent answer records unknown without demoting a status already persisted, and status_changed_at is when the change to the normalized status was observed -- a read time rather than the moment the status itself changed.



## OpenAPI

````yaml /api-reference/openapi.json get /v1/kyc
openapi: 3.1.0
info:
  summary: >-
...
paths:
  /v1/kyc:
    get:
      ...
      description: >-
        Returns the normalized identity-verification status, which this route is
        the single source of. Admits a customer token and answers synchronously.
        An unrecognized or absent answer records unknown without demoting a
        status already persisted, and status_changed_at is when the change to
        the normalized status was observed -- a read time rather than the moment
        the status itself changed.
      ...
      responses:
        '200':
          content:
            application/json:
              schema:
                $ref: '#/components/schemas/KycView'
...
components:
  schemas:
    KycView:
      ...
      description: The single source of the customer's verification status.
      properties:
        observed_at:
          description: When the status was last read.
          format: date-time
          title: Observed At
          type: string
        raw_status:
          anyOf:
            ...
            - type: 'null'
          description: >-
            The status string as stated, kept for support; null when none was
            stated.
          title: Raw Status
        status:
          description: >-
            The normalized verification status. unknown records an unrecognized
            or absent answer without demoting a status already persisted.
          enum:
            - not_started
            - in_progress
            - action_required
        ...
        status_changed_at:
          description: >-
            When the change to the normalized status was observed. The moment
            the status actually changed is not stated, so this is a read time
            rather than that moment.
          format: date-time
