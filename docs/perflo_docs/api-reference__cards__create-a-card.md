...
> Records a card creation and answers 202 with the operation that tracks it. Admits a customer token carrying a fresh iat, a Confirmation-Intent-ID naming the confirmed intent for this action, and an Idempotency-Key. The card is provisioned for the customer, so the only input is the customer's private nickname. The Idempotency-Key header binds for at least the replay window published by GET /v1/identity. Inside that guaranteed window an equal replay returns the same operation with Idempotent-Replayed set to true. A client with an unresolved response must stop before the window ends and reconcile operation history rather than retry later. A transport-uncertain write leaves the operation indeterminate and is never repeated.



## OpenAPI

````yaml /api-reference/openapi.json post /v1/cards
openapi: 3.1.0
info:
  summary: >-
...
paths:
  /v1/cards:
    post:
      ...
      description: >-
        Records a card creation and answers 202 with the operation that tracks
        it. Admits a customer token carrying a fresh iat, a
        Confirmation-Intent-ID naming the confirmed intent for this action, and
        an Idempotency-Key. The card is provisioned for the customer, so the
        only input is the customer's private nickname. The Idempotency-Key
...
components:
  schemas:
    CardCreate:
      ...
      description: |-
        The request that records a card issuance.

        The card is provisioned for the customer, so their private label is the
        only input.
      properties:
        nickname:
          anyOf:
            ...
            - type: 'null'
          description: >-
            The customer's private label for the card, trimmed and at most 80
            characters; a blank value is null.
          title: Nickname
      title: CardCreate
      type: object
    OperationView:
