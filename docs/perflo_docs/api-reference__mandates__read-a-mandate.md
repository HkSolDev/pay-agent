...
> Returns one mandate owned by the customer. A service-purchase mandate and its remaining rolling allowance are read entirely from local state. A grant-backed beneficiary mandate retains the reconciliation performed by the list route, and its remaining allowance counts only payments made under the mandate, so it is an upper bound on what the grant can still spend.



## OpenAPI

````yaml /api-reference/openapi.json get /v1/mandates/{mandate_id}
openapi: 3.1.0
info:
  summary: >-
...
paths:
  /v1/mandates/{mandate_id}:
    get:
      tags:
        - Mandates
      summary: Read a mandate
      description: >-
        Returns one mandate owned by the customer. A service-purchase mandate
        and its remaining rolling allowance are read entirely from local state.
        A grant-backed beneficiary mandate retains the reconciliation performed
        by the list route, and its remaining allowance counts only payments made
        under the mandate, so it is an upper bound on what the grant can still
        spend.
