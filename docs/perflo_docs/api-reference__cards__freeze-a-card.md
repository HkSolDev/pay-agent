...
paths:
  /v1/cards/{card_id}/freeze:
    post:
      ...
      description: >-
        Records a freeze on one of the customer's cards and answers 202 with the
        ...
        and an Idempotency-Key. The card takes freeze_pending while the write is
        submitted; a confirmed failure restores the previous status, and a
        transport-uncertain write leaves the card and its operation
        indeterminate and is never repeated. The Idempotency-Key header binds
