...
# Read the caller's verified identity

> Returns the identity derived from the caller's verified Perflo or pairing bearer token, plus the minimum idempotency replay guarantee.



## OpenAPI

````yaml /api-reference/openapi.json get /v1/identity
openapi: 3.1.0
info:
  summary: >-
...
paths:
  /v1/identity:
    get:
      tags:
        - Identity
      summary: Read the caller's verified identity
      description: >-
        Returns the identity derived from the caller's verified Perflo or
        pairing bearer token, plus the minimum idempotency replay guarantee.
...
components:
  schemas:
    IdentityView:
      ...
      properties:
        actor_type:
          description: The authority class authenticated for this request.
          enum:
            - customer
            - agent
          title: Actor Type
        ...
        client_id:
          anyOf:
            - type: string
            - type: 'null'
          description: The paired-agent identifier for an agent token; null for a customer.
          title: Client Id
        idempotency_replay_window_seconds:
          description: >-
            The minimum time replay protection is guaranteed for an idempotency
            key accepted by a new financial mutation.
          exclusiveMinimum: 0
          title: Idempotency Replay Window Seconds
          type: integer
        scopes:
          description: The caller's effective scopes, sorted.
          items:
            type: string
          title: Scopes
          type: array
        server_time:
          description: The UTC time this identity response was constructed.
          format: date-time
          title: Server Time
          type: string
        subject:
          description: The stable subject for the caller.
          title: Subject
          type: string
        wallet:
          anyOf:
            ...
            - type: 'null'
          description: >-
            The verified Perflo wallet claim for a customer token; null for an
            agent.
          title: Wallet
      required:
        - subject
