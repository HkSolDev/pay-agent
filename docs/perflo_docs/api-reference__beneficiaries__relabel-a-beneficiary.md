...
# Relabel a beneficiary

> Changes or clears the customer's private label and returns the updated beneficiary. Admits a customer token and answers synchronously. A null or blank nickname clears the label. Answers 409 when another beneficiary already carries the requested label.



## OpenAPI

````yaml /api-reference/openapi.json patch /v1/beneficiaries/{beneficiary_id}
openapi: 3.1.0
info:
  summary: >-
...
paths:
  /v1/beneficiaries/{beneficiary_id}:
    patch:
      tags:
        - Beneficiaries
      summary: Relabel a beneficiary
      description: >-
        Changes or clears the customer's private label and returns the updated
        beneficiary. Admits a customer token and answers synchronously. A null
        or blank nickname clears the label. Answers 409 when another beneficiary
        already carries the requested label.
      operationId: rename_beneficiary
      parameters:
        - in: path
          name: beneficiary_id
          required: true
      ...
      requestBody:
        content:
          application/json:
            schema:
              $ref: '#/components/schemas/BeneficiaryNicknameUpdate'
        required: true
      responses:
        '200':
          content:
...
components:
  schemas:
    BeneficiaryNicknameUpdate:
      additionalProperties: false
      description: The request that changes or clears a beneficiary's private label.
      properties:
        nickname:
          anyOf:
            - maxLength: 80
      ...
      required:
        - nickname
      title: BeneficiaryNicknameUpdate
      type: object
    BeneficiaryView:
      additionalProperties: false
      description: |-
