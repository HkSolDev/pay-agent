...
> Starts a hosted card-verification session and answers with the normalized status, the verification token, the applicant identifier and the action to open. Admits a customer token and answers synchronously.



## OpenAPI

````yaml /api-reference/openapi.json post /v1/card-account/kyc-sessions
openapi: 3.1.0
info:
  summary: >-
...
tags:
  ...
  - description: >-
    ...
    name: Webhooks
paths:
  /v1/card-account/kyc-sessions:
    post:
      tags:
        - Cards
      summary: Start card verification
      description: >-
        Starts a hosted card-verification session and answers with the
        normalized status, the verification token, the applicant identifier and
        the action to open. Admits a customer token and answers synchronously.
      operationId: create_card_kyc_session
      responses:
        '201':
          content:
            application/json:
              schema:
                $ref: '#/components/schemas/CardKycSessionView'
...
components:
  schemas:
    CardKycSessionView:
      ...
      description: A hosted card-verification session.
      properties:
        action:
          $ref: '#/components/schemas/RedirectAction'
          description: The hosted verification session to open.
        applicant_id:
          description: The verification applicant identifier.
          title: Applicant Id
          type: string
        kyc_raw_status:
          anyOf:
        ...
        kyc_status:
          description: The normalized card-verification status at session creation.
          enum:
            - not_started
            - in_progress
            - action_required
        ...
        verification_token:
          description: >-
            The credential for one hosted card-verification session. It is not a
            reusable customer-account access or refresh token. Do not log or
            persist it.
