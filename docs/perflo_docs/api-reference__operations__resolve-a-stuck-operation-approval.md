...
# Resolve a stuck operation approval

> Settles one browser approval that could not be reconciled automatically and returns the operation. Admits a customer token carrying a fresh iat and an Idempotency-Key. The body attests what Perflo did: grant_created finds the one new grant that still matches the stored approval exactly, or optionally names it for clients that can read the grant identifier; no_grant_created retires the operation, and is refused while Perflo still lists any grant matching the approval. Both attestations are checked against a live read and refused on any disagreement, and neither submits a payment: an adopted grant resumes the same queued execution the automatic path uses. The route is available only once automatic reconciliation has stopped. The Idempotency-Key header binds for at least the replay window published by GET /v1/identity. Inside that guaranteed window an equal replay returns the same operation with Idempotent-Replayed set to true. A client with an unresolved response must stop before the window ends and reconcile operation history rather than retry later.



## OpenAPI

````yaml /api-reference/openapi.json post /v1/operations/{operation_id}/approval/resolution
openapi: 3.1.0
info:
  summary: >-
...
paths:
  /v1/operations/{operation_id}/approval/resolution:
    post:
      tags:
        - Operations
      summary: Resolve a stuck operation approval
      description: >-
        Settles one browser approval that could not be reconciled automatically
        and returns the operation. Admits a customer token carrying a fresh iat
        and an Idempotency-Key. The body attests what Perflo did: grant_created
        finds the one new grant that still matches the stored approval exactly,
        or optionally names it for clients that can read the grant identifier;
        no_grant_created retires the operation, and is refused while Perflo
        still lists any grant matching the approval. Both attestations are
        checked against a live read and refused on any disagreement, and neither
        submits a payment: an adopted grant resumes the same queued execution
        the automatic path uses. The route is available only once automatic
        reconciliation has stopped. The Idempotency-Key header binds for at
        ...
        than retry later.
      operationId: resolve_operation_approval
      parameters:
        - in: path
          name: operation_id
          required: true
          ...
          application/json:
            schema:
              discriminator:
                mapping:
                  grant_created:
                    $ref: '#/components/schemas/ApprovalResolutionGrantCreated'
                  no_grant_created:
                    $ref: '#/components/schemas/ApprovalResolutionNoGrant'
...
components:
  schemas:
    ApprovalResolutionGrantCreated:
      additionalProperties: false
      description: |-
        The customer's attestation that Perflo did create the approved grant.

        The account is re-read and one grant adopted only when it is new,
        active, unused, and the unique exact match for the approval.
      properties:
        attestation:
          const: grant_created
          description: Selects the attestation that the approval produced a grant.
          title: Attestation
          type: string
        grant_id:
          anyOf:
            ...
            - type: 'null'
          description: >-
            An optional beneficiary-grant identifier for clients that can read
            it. When omitted, one unique new grant matching the stored approval
            is required. In both cases, the grant must be active, unused, and
            unclaimed.
          title: Grant Id
      required:
        - attestation
      title: ApprovalResolutionGrantCreated
      type: object
    ApprovalResolutionNoGrant:
      additionalProperties: false
      description: >-
        The customer's attestation that Perflo created no grant.


        Whether Perflo created one cannot be determined automatically, so the
        account is re-read and the

        operation retired only when no grant matching the approval exists.
      properties:
        attestation:
          const: no_grant_created
          description: Selects the attestation that the approval produced no grant.
          title: Attestation
          type: string
      required:
        - attestation
      title: ApprovalResolutionNoGrant
      type: object
    OperationView:
      additionalProperties: false
      description: >-
