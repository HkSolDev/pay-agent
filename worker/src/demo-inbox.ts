...
const scenarios: DemoScenario[] = [
  ...
  { name: "multiline-pdf", description: "Invoice fields split across PDF lines", message: message("multiline-pdf", "Acme Billing <billing@acme.example>", "Invoice attached", pdfPayload("pdf-multiline")), pdfText: "ACME SERVICES\nTAX INVOICE\nInvoice No: AC-2026-44\nTotal Amount Due:\n₹18,750\nUPI: acme@okaxis", expectedClassification: "invoice" },
  { name: "german-pdf", description: "German invoice PDF", message: message("german-pdf", "Rechnung <rechnung@de.example>", "Rechnung RE-2026-9", pdfPayload("pdf-german")), pdfText: "RECHNUNG\nRechnungsnummer: RE-2026-9\nGesamtbetrag: 500 EUR\nZahlungsziel: 30 Tage", expectedClassification: "invoice" },
  { name: "scanned-pdf", description: "Image-only/scanned PDF with no extractable text", message: message("scanned-pdf", "Scans <scans@vendor.example>", "Invoice scan", pdfPayload("pdf-scanned")), pdfText: null, expectedClassification: "invoice" },
  { name: "corrupt-pdf", description: "Corrupt PDF extraction failure", message: message("corrupt-pdf", "Billing <billing@vendor.example>", "Invoice INV-404", pdfPayload("pdf-corrupt")), pdfText: "__CORRUPT__", expectedClassification: "invoice" },
